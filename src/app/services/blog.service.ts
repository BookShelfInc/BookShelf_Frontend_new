...
export class BlogService {
    ...
    getAllPosts(): Observable<Post[]> {
        let url = 'http://bookshelf.life:8080/blog/post/all/';
    ...
    deletePost(id: number) {    
        let url = 'http://bookshelf.life:8080/blog/post/delete/' + id.toString() + '/';
        return this.http.post(url, this.jwt()).map((res: Response) => res);
    }

    //TODO: Set button
    deleteComment(id: number) {
        let url = 'http://bookshelf.life:8080/blog/post/comment/delete/' + id.toString() + '/';
        let headers = new Headers({ 'Content-Type': 'application/json' });
        let options = new RequestOptions({ headers: headers });
    ...
    getUserPosts(id: number): Observable<Post[]> {
        let url = 'http://bookshelf.life:8080/blog/post/user/' + id.toString() + '/';
        let headers = new Headers({ 'Content-Type': 'application/json' });
        let options = new RequestOptions({ headers: headers });
        return this.http.get(url, options).map((res: Response) => res.json());
    }
    ...
    getPost(id: number): Observable<Post> {
        let url = 'http://bookshelf.life:8080/blog/post/' + id.toString() + '/';
        let headers = new Headers({ 'Content-Type': 'application/json' });
        let options = new RequestOptions({ headers: headers });
        return this.http.get(url, options).map((res: Response) => res.json());
    }
